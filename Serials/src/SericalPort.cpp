...
void Serial::pack(float pitch, float yaw, float dist, uint8_t centerLock, uint8_t targetMode,uint8_t buffMode)
{
    ...
    buff[14] = static_cast<char>((centerLock) & 0XFF);
    buff[15] = static_cast<unsigned char>((targetMode) & 0XFF);
    buff[16] = static_cast<char>((buffMode) & 0XFF);
    buff[17] = static_cast<char>((0x0000));
    buff[18] = static_cast<char>(0x0000);
    buff[19] = static_cast<char>((CRC8) & 0XFF);
}
